...
while 1:
    ...
    while esc1.inWaiting() > 0 or esc2.inWaiting() > 0  or esc3.inWaiting() > 0  or esc4.inWaiting() > 0  or esc5.inWaiting() > 0   or esc6.inWaiting() > 0  or esc7.inWaiting() > 0  or esc8.inWaiting() > 0 :
        print("reading ESC 1")
        ReadDataBuffer(esc1.read_until(b'\xff\xff'))
        print("reading ESC 2")
        ReadDataBuffer(esc2.read_until(b'\xff\xff'))
        print("reading ESC 3")
        ReadDataBuffer(esc3.read_until(b'\xff\xff'))
        print("reading ESC 4")
        ReadDataBuffer(esc4.read_until(b'\xff\xff'))
        print("reading ESC 5")
        ReadDataBuffer(esc5.read_until(b'\xff\xff'))
        print("reading ESC 6")
        ReadDataBuffer(esc6.read_until(b'\xff\xff'))
        print("reading ESC 7")
        ReadDataBuffer(esc7.read_until(b'\xff\xff'))
        print("reading ESC 8")
        ReadDataBuffer(esc8.read_until(b'\xff\xff'))
        esc1.flushInput()
        esc2.flushInput()
        esc3.flushInput()
        esc4.flushInput()
